...
run_dir=${1:-"$ROOT/results/deepseek_v4_csa_topk/latest"}
watch_mode=${2:-}
...
check_once() {
  local now state k mode pid newest newest_mtime age health gpu_busy
  now=$(date +%s)
  state=$(read_state state); k=$(read_state k); mode=$(read_state mode); pid=$(read_state server_pid)
  printf 'run=%s state=%s k=%s mode=%s pid=%s\n' "$run_dir" "${state:-unknown}" "${k:--}" "${mode:--}" "${pid:--}"
