...
The SPS table is required for a meaningful dynamic boundary; an uninitialized table degenerates to verify-all. The scripts restart the server for every run, remove stale JSONL output, use seeds starting at `BASE_SEED`, and keep all other settings fixed. Override `TP_SIZE`, `RUN_COUNT`, `NUM_PROMPTS`, `MAX_CONCURRENCY`, `REQUEST_RATE`, or `EDGE_MARGIN` before running. Use `EXTRA_SERVER_ARGS`, `EXTRA_SPS_ARGS`, and `EXTRA_BENCH_ARGS` for flags without dedicated variables; for example, export `EXTRA_SERVER_ARGS="--enable-hisparse"` to collect DeepSeek-V4 CSA misses.
